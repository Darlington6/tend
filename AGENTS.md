...
- **Monetization:** RevenueCat (`react-native-purchases` + `react-native-purchases-ui`), configured in `lib/purchases.ts`, entitlement state in `lib/entitlements-store.ts`, wired up by `components/PurchasesManager.tsx`. RevenueCat identity is linked to the Supabase user id on sign-in so a subscription follows the account across devices. The entitlement identifier is `premium` (`constants/revenuecat.ts`), it must match whatever is configured in the RevenueCat dashboard. Since these are native modules, **Expo Go no longer works** for this project, use a custom dev client (`npx expo run:ios` / `npx expo run:android`, or an EAS development build).
